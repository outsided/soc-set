...
import {forSettings,forTeams} from './data.settings'


export default function Settings(){
    const [teams, setTeams] = useState<forTeams[]>([])
    ...
    return (
        <div className="flex m-[30px] w-full justify-center">
            <div className="text-white  ">
                <ul className="font-style: italic text-xl flex flex-col cursor-pointer ">
                {forSettings.map(item=> <li key={item.id} className="m-[40px]" onClick={()=> settingsHeandler(item.num)}>{item.setSettin}</li>)}
                </ul>
            </div>
            <div>
                <ul className="flex text-white text-[20px] ml-[40px] text-center">
